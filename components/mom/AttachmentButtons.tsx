...
import { getFileIcon, getFileTypeLabel } from '@/types/mom/file';
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (result.success && result.data) {
        ...
        // Excel, Word, PowerPoint, and other files will use 'file' type
            ...
            {attachments.map(attachment => (
              <div key={attachment.id} className="relative w-32 h-24 border rounded overflow-hidden group">
                {attachment.type === 'image' ? (
                  ...
                  <a 
                    href={attachment.blobUrl || attachment.driveUrl || attachment.data}
                    download={attachment.fileName}
                    className="w-full h-full flex flex-col items-center justify-center bg-gray-100 p-2 relative hover:bg-gray-200 transition-colors"
                  >
                    <div className="text-3xl mb-1">
                      {getFileIcon(attachment.fileName || 'file')}
                    </div>
                    <span className="text-xs text-gray-600 text-center truncate w-full px-1" title={attachment.fileName}>
                      {attachment.fileName || 'File'}
                    </span>
                    <span className="absolute top-1 left-1 text-xs bg-black bg-opacity-70 text-white px-1 rounded">
