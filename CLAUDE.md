...
**以下のルールは必ず守ること:**
1. **モックデータ・デモデータ禁止**
   - モックAPI、テストデータ、サンプルデータは一切使用しない
   - MOCK_TERMS、MOCK_DATAなどのダミーデータを作成・使用しない
   - 開発環境でも本番環境でも実データのみを扱う
   - テスト用のエンドポイント（translate-mock等）を作成しない

2. **実データ・実APIの使用**
   - 常に実際のAPIサービス（Gemini API、OpenAI API等）を使用
   - 常に実際のデータベース（Supabase、Prisma）に接続
   - データが無い場合は空の状態を表示（ダミーデータで埋めない）

3. **デバッグ・開発時の注意**
   - console.logは必要最小限に留める
   - process.env.NODE_ENV による分岐でモックを使わない
   - URLパラメータ（?mock=true等）でのモック切替を実装しない
