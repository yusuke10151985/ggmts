...
function createStructureContent(
  ...
  items.forEach(item => {
    ...
    if (item.level === 4) {
      ...
      if (item.details) {
        content.push({
          text: `Action Details: ${item.details}`,
          style: ['normal', detailIndent],
          font: FontUtils.getFont(item.details)
        });
        
        if (item.detailsTranslations) {
          const detailTranslations = createTranslationContent(item.detailsTranslations);
          detailTranslations.forEach(dt => {
            if (typeof dt === 'object' && 'style' in dt) {
              dt.style = Array.isArray(dt.style)
                ? [...dt.style, detailIndent]
                : [dt.style as string, detailIndent];
            }
          });
          content.push(...detailTranslations);
        }
      }
