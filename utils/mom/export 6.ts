...
function generateStructureText(items: StructureItem[], indent: string = ''): string {
  ...
  items.forEach(item => {
    ...
    if (item.level === 4) {
      // Add action details
      if (item.details) {
        text += `${indent}   Action Details: ${item.details}\n`;
      }
...
function generateStructureMarkdown(items: StructureItem[], level: number = 0, currentRevision?: number): string {
  ...
  items.forEach(item => {
    ...
    if (item.level === 4) {
      // Add action details
      if (item.details && item.detailsTranslations) {
        // **SHOW ONLY TRANSLATIONS**: Hide original text, show only translations
        markdown += `### **Action Details: ${item.detailsTranslations.en}**  \n`;
        markdown += `> **JA: ${item.detailsTranslations.ja}**  \n`;
        markdown += `> **TH: ${item.detailsTranslations.th}**  \n`;
