...
function drawStructureItems(
  ...
  for (const item of items) {
    ...
    if (item.level === 4) {
      if (item.details) {
        drawText(ctx, `Details: ${item.details}`, { fontSize: 10, indent: indent + 15 });
        
        if (item.detailsTranslations) {
          drawText(ctx, `EN: ${item.detailsTranslations.en}`, { fontSize: 9, color: rgb(0.4, 0.4, 0.4), indent: indent + 20 });
          drawText(ctx, `JA: ${item.detailsTranslations.ja}`, { fontSize: 9, color: rgb(0.4, 0.4, 0.4), indent: indent + 20 });
          drawText(ctx, `TH: ${item.detailsTranslations.th}`, { fontSize: 9, color: rgb(0.4, 0.4, 0.4), indent: indent + 20 });
        }
      }
