...
export function generatePDFFilename(mom: MOM): string {
  // MOM IDはそのまま使用（MOM-プレフィックスを含む）
  const momId = mom.momId;
  const revision = `Rev.${mom.revision}`;
  const status = mom.status;
  ...
  const cleanTitle = (title: string) => {
    return title
      ...
      .replace(/_+/g, '_')
      .replace(/^_|_$/g, '')
      .substring(0, 50); // 長さ制限を50文字に拡大
  };
  
  return `${momId}_${revision}_${status}_${cleanTitle(enTitle)}_${cleanTitle(jpTitle)}_${cleanTitle(thTitle)}.pdf`;
}
