...
import { MOM, StructureItem } from '@/types';
...
import { getRevisionHexColor, isModifiedInRevision, compareMOMs } from '@/lib/revision-utils';
import { generateHierarchicalNumbers } from '@/lib/numbering-utils';
...
function generateStructureText(items: StructureItem[], indent: string = ''): string {
  ...
  items.forEach(item => {
    // **HIERARCHICAL NUMBERING**: Make numbering more prominent
    text += `${indent}[${item.number}] ${item.title}\n`;
    
    if (item.level === 4) {
      // Add action details
      if (item.details) {
        text += `${indent}   Details: ${item.details}\n`;
      }
...
export function exportToMarkdown(mom: MOM): string {
  ...
  // **TRANSLATION IN EXPORT**: Include translations in markdown export
  let markdown = `# MOM: ${mom.title}`;
  if (mom.titleTranslations) {
    markdown += ` | EN: ${mom.titleTranslations.en} | JA: ${mom.titleTranslations.ja} | TH: ${mom.titleTranslations.th}`;
  }
  markdown += `\n\n`;
  ...
  // Metadata
  markdown += `**MOM ID:** ${mom.momId}  \n`;
  markdown += `**Revision:** ${mom.revision}  \n`;
  markdown += `**Date:** ${mom.date}  \n`;
  markdown += `**Status:** ${mom.status}  \n\n`;
...
function generateStructureMarkdown(items: StructureItem[], level: number = 0, currentRevision?: number): string {
  ...
  items.forEach(item => {
    ...
    if (isModified) {
      // **リビジョン変更の太字表示**: 変更されたアイテムはタイトル全体を太字で表示
      markdown += `${indent}${bullet}${prefix} **[${item.number}] ${item.title} [Rev.${currentRevision}]**`;
    } else {
      markdown += `${indent}${bullet}${prefix} **[${item.number}]** ${item.title}`;
    }
    // **TRANSLATION IN EXPORT**: Add translations for each item
    if (item.translations) {
      markdown += ` | EN: ${item.translations.en} | JA: ${item.translations.ja} | TH: ${item.translations.th}`;
    }
    markdown += `\n\n`;
    ...
    if (item.level === 4) {
      ...
      if (item.details) {
        ...
        // **TRANSLATION IN EXPORT**: Add detail translations
        if (item.detailsTranslations) {
          markdown += ` | EN: ${item.detailsTranslations.en} | JA: ${item.detailsTranslations.ja} | TH: ${item.detailsTranslations.th}`;
        }
        markdown += `  \n`;
      }
      if (item.responsibleParties && item.responsibleParties.length > 0) {
...
async function addStructureToPDF(
  ...
  for (const item of items) {
    ...
    pdf.setFontSize(11);
    const itemText = `${' '.repeat(indent * 2)}${item.number}. ${item.title}`;
    ...
    if (item.level === 4) {
      ...
      if (item.details) {
        const detailLines = pdf.splitTextToSize(`Details: ${item.details}`, 170 - indent * 5);
        detailLines.forEach((line: string) => {
          if (yPosition > 270) {
            pdf.addPage();
            yPosition = 20;
          }
          pdf.text(line, 25 + indent * 5, yPosition);
          yPosition += 5;
        });
        yPosition += 2;
      }
...
export async function exportToPDF(mom: MOM): Promise<Blob> {
  // **2025-07-11 PDFMAKE**: Use pdfmake for better Unicode support
  // pdfmake has better built-in support for multi-language PDFs
  try {
    const { exportToPDFWithPdfMake } = await import('./export-pdf-pdfmake');
    return await exportToPDFWithPdfMake(mom);
  } catch (error) {
    console.warn('Failed to use pdfmake, falling back to simple export:', error);
    // Fallback to simple export that safely handles Unicode
...
export { generatePDFFilename, generateMarkdownFilename };