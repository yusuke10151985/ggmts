...
function addStructureToGoogleDocs(
  ...
  items.forEach(item => {
    ...
    const headingStyle = item.level <= 2 ? STYLES.heading2 : STYLES.heading3;
...
function generateStructureHTML(
  ...
  items.forEach(item => {
    ...
    const headingClass = item.level <= 2 ? 'heading2' : 'heading3';
    ...
    if (item.level === 4) {
      ...
      if (item.responsibleParties && item.responsibleParties.length > 0) {
        ...
        html += `<p class="normal ${detailIndentClass}"><strong>Responsible:</strong> ${escapeHtml(names)}</p>`;
      }
      
      if (item.dueDate) {
        html += `<p class="normal ${detailIndentClass}"><strong>Due Date:</strong> ${escapeHtml(item.dueDate)}</p>`;
      }
      
      if (item.status) {
        html += `<p class="normal ${detailIndentClass}"><strong>Status:</strong> ${escapeHtml(item.status.toUpperCase())}</p>`;
      }
