...
export function exportToMarkdown(mom: MOM): string {
  ...
  if (mom.titleTranslations) {
    ...
    markdown += mom.title; // Fallback to original if no translations
  }
  markdown += `\n\n`;
  
  // **MEETING GOAL**: Show only translations, hide original text
  if (mom.goal && mom.goalTranslations) {
    markdown += `## Meeting Goal\n> EN: ${mom.goalTranslations.en}  \n> JA: ${mom.goalTranslations.ja}  \n> TH: ${mom.goalTranslations.th}\n\n`;
  } else if (mom.goal) {
...
function generateStructureMarkdown(items: StructureItem[], level: number = 0, currentRevision?: number): string {
  ...
  items.forEach(item => {
    ...
    if (item.translations) {
      if (isModified) {
        markdown += `${indent}${bullet}${prefix} **[${item.number}] EN: ${item.translations.en} | JA: ${item.translations.ja} | TH: ${item.translations.th} [Rev.${currentRevision}]**`;
      } else {
        markdown += `${indent}${bullet}${prefix} **[${item.number}]** EN: ${item.translations.en} | JA: ${item.translations.ja} | TH: ${item.translations.th}`;
      }
    } else {
      // Fallback to original if no translations
      if (isModified) {
    ...
    if (item.level === 4) {
      // Add action details
      if (item.details) {
        markdown += `**Action Details:** ${item.details}`;
...
async function addStructureToPDF(
  ...
  for (const item of items) {
    ...
    if (item.level === 4) {
      ...
      if (item.details) {
        const detailLines = pdf.splitTextToSize(`Action Details: ${item.details}`, 170 - indent * 5);
