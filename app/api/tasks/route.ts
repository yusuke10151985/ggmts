...
export async function GET(request: NextRequest) {
  try {
    ...
    const rows = await getSheetData('Tasks!A2:I');
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get existing tasks
    const existingRows = await getSheetData('Tasks!A2:I');
    ...
    for (const update of updatedTasks) {
      await updateSheetData(`Tasks!A${update.row}:I${update.row}`, [update.data]);
    }
