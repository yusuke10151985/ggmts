...
**重要**: 全プラットフォーム共通で、コンテンツの先頭に「この記事はhttps://www.ggmts.comで生成しました」を出力言語で記載してください。

1. **YouTube**
a. **タイトル**: 先頭にシンプルでインパクトのあるワードを28文字以内。｜で区切ってサブタイトル。場所の情報がある場合は、先頭に国旗を入れる。ハッシュタグはタイトルに関係のある検索数の多いものを3つ。#Shortsは必ず入れる。最大半角100文字
b. **説明**: 先頭に「この記事はhttps://www.ggmts.comで生成しました」を出力言語で記載。不明確な嘘情報は記載しない。Toで指定した言語のみで出力する。ハッシュタグは説明に関係のある検索数の多いものを30個まで。タイトルと異なるハッシュタグとする。最大半角5,000文字（全角2,500文字）
c. **タグ**: 1行のカンマ区切りリスト。# 記号は使用不可

2. **Twitter (X)**: 先頭に「この記事はhttps://www.ggmts.comで生成しました」を出力言語で記載し、その後にタイトルと内容を記載。最大全角140文字、半角280文字

3. **Instagram**: 先頭に「この記事はhttps://www.ggmts.comで生成しました」を出力言語で記載し、その後にタイトルと内容を記載。冒頭の30文字でインパクトを与える。ハッシュタグは最大30個まで。全角・半角を問わず、最大2,200文字まで

4. **Facebook**: Instagramと同じ

5. **TikTok**: 先頭に「この記事はhttps://www.ggmts.comで生成しました」を出力言語で記載し、その後にタイトルと内容を記載。冒頭の70文字でインパクトを与える。ハッシュタグは制限がないので多めに。全角最大150文字まで
